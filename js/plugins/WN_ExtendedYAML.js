 ...
 * 0 - This Event (May behave weirdly)
 * -1 - Player
 * -2,-3,-4,etc. - Followers
 ...
 * - Windowskins work.
 * - Facesets work, but Geo's faceChange plugin doesn't.
 ...
 * 
 * @param console
 * @text Console.log EVERYWHERE
 * @desc Console.log every single parameter and shit
 * @type boolean
 * @default false
 * 
 */
...
Imported.WN_ExtendedYAML = true;

(function() {

// Defining shit
var WN = WN || {};
WN.ExtYAML = WN.ExtYAML || {};

// Params
WN.ExtYAML.Param = PluginManager.parameters('WN_ExtendedYAML');

// Macros
WN.ExtYAML.marcoyaml = WN.ExtYAML.Param["macroyaml"]

// BannedScenes
WN.ExtYAML.bannedScenes = JSON.parse(WN.ExtYAML.Param["bannedscenes"])

// Windowskins
WN.ExtYAML.reservedwindowskins = JSON.parse(WN.ExtYAML.Param["reservedwindowskins"])
WN.ExtYAML.windowskin = WN.ExtYAML.Param["windowskin"]

// Text Sounds
WN.ExtYAML.textsound = WN.ExtYAML.Param["textsound"]
WN.ExtYAML.textvolume = Number(WN.ExtYAML.Param["textvolume"])
WN.ExtYAML.textpitch = Number(WN.ExtYAML.Param["textpitch"])
WN.ExtYAML.textpitchvar = Number(WN.ExtYAML.Param["textpitchvar"])
WN.ExtYAML.textpan = Number(WN.ExtYAML.Param["textpan"])
WN.ExtYAML.textpanvar = Number(WN.ExtYAML.Param["textpanvar"])
WN.ExtYAML.textinterval = Number(WN.ExtYAML.Param["textinterval"])

// SE
WN.ExtYAML.svolume = Number(WN.ExtYAML.Param["svolume"])
WN.ExtYAML.spitch = Number(WN.ExtYAML.Param["spitch"])
WN.ExtYAML.span = Number(WN.ExtYAML.Param["span"])

// ME
WN.ExtYAML.mvolume = Number(WN.ExtYAML.Param["mvolume"])
WN.ExtYAML.mpitch = Number(WN.ExtYAML.Param["mpitch"])
WN.ExtYAML.mpan = Number(WN.ExtYAML.Param["mpan"])

// Background
WN.ExtYAML.background = Number(WN.ExtYAML.Param["background"])
WN.ExtYAML.facebackgroundcolor = WN.ExtYAML.Param["facebackgroundcolor"]

// TRain Msg Shapes
WN.ExtYAML.windowx = WN.ExtYAML.Param["windowx"]
WN.ExtYAML.windowy = WN.ExtYAML.Param["windowy"]
WN.ExtYAML.windowwidth = WN.ExtYAML.Param["windowwidth"]
WN.ExtYAML.windowheight = WN.ExtYAML.Param["windowheight"]
WN.ExtYAML.windowquickname = eval(WN.ExtYAML.Param["windowquickname"])

// Mirror
WN.ExtYAML.mirror = eval(WN.ExtYAML.Param["mirror"])

// Position Types
WN.ExtYAML.positionType = Number(WN.ExtYAML.Param["positionType"])
WN.ExtYAML.positionTypeCaption = Number(WN.ExtYAML.Param["positionTypeCaption"])

// GalvCaptions
WN.ExtYAML.target = Number(WN.ExtYAML.Param["target"])
WN.ExtYAML.time = Number(WN.ExtYAML.Param["time"])
WN.ExtYAML.delay = Number(WN.ExtYAML.Param["delay"])
WN.ExtYAML.arrows = eval(WN.ExtYAML.Param["arrows"])

// Console Logging Nightmare
WN.ExtYAML.console = eval(WN.ExtYAML.Param["console"])
...
Game_Message.prototype.showLanguageMessage = function(code) {
  ...
  var macro = data.macro ? LanguageManager.getMessageData(`${WN.ExtYAML.marcoyaml}.${data.macro}`) : {}
  var base = WN.ExtYAML
  ...
  //yamlConsole(windowskin)
  if (HimeWindowSkin && windowskin && type != 'caption' && type != 'popup' && !WN.ExtYAML.bannedScenes.includes(SceneManager._scene.constructor.name)) {
  ...
  //if (!extraFaces && $gameVariables.value(1930) === 1 && ImageManager.inGroup(faceset,1)) {
  //  extraFaces = [{faceset: faceset+"2", faceindex: faceindex}];
  //};
  console.log(extraFaces);
  ...
  //These don't use TR.NullCoal due to null nonsense
  var windowx = data.windowx !== undefined ? String(data.windowx) === 'null' ? null : Number(data.windowx) : macro.windowx !== undefined ? String(macro.windowx) === 'null' ? null : Number(macro.windowx) : String(WN.ExtYAML.windowx) === 'null' ? null : Number(WN.ExtYAML.windowx);
  var windowy = data.windowy !== undefined ? String(data.windowy) === 'null' ? null : Number(data.windowy) : macro.windowy !== undefined ? String(macro.windowy) === 'null' ? null : Number(macro.windowy) : String(WN.ExtYAML.windowy) === 'null' ? null : Number(WN.ExtYAML.windowy);
  var windowwidth = data.windowwidth !== undefined ? String(data.windowwidth) === 'null' ? null : Number(data.windowwidth) : macro.windowwidth !== undefined ? String(macro.windowwidth) === 'null' ? null : Number(macro.windowwidth) : String(WN.ExtYAML.windowwidth) === 'null' ? null : Number(WN.ExtYAML.windowwidth);
  var windowheight = data.windowheight !== undefined ? String(data.windowheight) === 'null' ? null : Number(data.windowheight) : macro.windowheight !== undefined ? String(macro.windowheight) === 'null' ? null : Number(macro.windowheight) : String(WN.ExtYAML.windowheight) === 'null' ? null : Number(WN.ExtYAML.windowheight);
  var windowquickname = data.windowquickname || macro.windowquickname || WN.ExtYAML.windowquickname;
  if (TRainMsgShape) {
  ...
  // TOKI PONA
  var tokipona = TR.NullCoal([data.tokipona,macro.tokipona],false);
  if (tokipona) {
    text = this.tokipona(text)
  }
  ...
  if (positionType == 'TBD') {
    positionType = ((type == "popup") || (type == "caption")) ? WN.ExtYAML.positionTypeCaption : WN.ExtYAML.positionType
  };
...
ImageManager.reserveSystem("Window", 0, this._imageReservationId); //Even tho this is in the default I don't trust people.
  for (i=0;i<WN.ExtYAML.reservedwindowskins.length;i++) {
   if (systemFileExists(WN.ExtYAML.reservedwindowskins[i])) { // I also don't trust that the file they listed actually exists
      ImageManager.reserveSystem(WN.ExtYAML.reservedwindowskins[i],0,this._imageReservationId);
   } else {
    console.warn(`Idiot Alert: ${WN.ExtYAML.reservedwindowskins[i]} doesn't exist, yet SOMEONE tried putting it in the reservation list in WN_ExtendedYAML`)
   }
  }
};
...
Game_Message.prototype.haiku = function() {
    ...
    const seven = LanguageManager.getMessageData("prl_dev_room.haiku_seven");
    const five = LanguageManager.getMessageData("prl_dev_room.haiku_five");
    let haiku = 
...
// Toki pona
Game_Message.prototype.tokipona = function(oldtext) {
  const words = [
  ...
  'kijetesantakalu','soko','meso','epiku','kokosila','lanpan','n','misikeke','ku','apeja','majuna','linluwi','sewi2','tokipona','.',':',
  //These extra symbols are technically toki but are for non-TP use
  'up','down','right','left','upleft','upright','downright','downleft','space','star','emittersleft','emittersright','smile','frown','warning'
  ]
  var text = oldtext.split(' ');
  //yamlConsole(text)
  var newtext = ''
  for (let i = 0; i < text.length; i++) {
    if (text[i].includes('\\') || text[i].includes('<br>')) { newtext += text[i]; continue; }
    let fromWords = (element) => element === text[i]
    let index = words.findIndex(fromWords);
    if (index > -1) { newtext += `\\i[${index + 32}]`; continue; }
    newtext += text[i]; continue;
  }
  return newtext;
}

//This is so that This Event in GalvCaptions works correctly.
WN.ExtYAML.pluginCommand = Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args) {
  ...
  return WN.ExtYAML.pluginCommand.call(this, command, args);
};
