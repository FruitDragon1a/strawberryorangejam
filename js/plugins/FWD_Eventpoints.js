...
@plugindesc v1.0.2 Add event points to audio files for event triggering.
@help
...

{
    ...
    const ExtendedChecks = ["audio/me/prl_credits_ch1.ogg"];
    const selectBuffer = function() {
        if (AudioManager._meBuffer && AudioManager._meBuffer.isPlaying()) return AudioManager._meBuffer;
    ...
    WebAudio.prototype._readMetaData = function (array, index, size) {
        //console.log(array,index,size);
        if (ExtendedChecks.includes(this._url)) size = 700;
        this._PingPoints = {};
    ...
    const EPSILON = 0.05
    FWD.IsAnyPingPointActive = function () {
        let buffer = selectBuffer();
        if (!buffer) return false;
        if (!buffer.isPlaying()) return false;
        let currentTime = buffer.seek();

        for (let pinglist of buffer._PingPoints) {
            for (let point of pinglist) {
                if (FWD.pastPoint) return currentTime >= point;
    ...
    FWD.IsPingPointActive = function (pingname) {
        let buffer = selectBuffer();
        if (!buffer) return false;
        if (!buffer.isPlaying()) return false;
        return buffer._TriggeredPingPoints[pingname] === true;
    }
    FWD._IsPingPointActive = function (pingname) {
        let buffer = selectBuffer();
        if (!buffer) return false;
        if (!buffer.isPlaying()) return false;
        let currentTime = buffer.seek();
    ...
    FWD.IsAnonymousPingPointActive = function () {
        let buffer = selectBuffer();
        if (!buffer) return false;
        if (!buffer.isPlaying()) return false;
        return buffer._TriggeredPingPoints['__ANONYMOUS__'] === true;
    }
    FWD._IsAnonymousPingPointActive = function () {
        return FWD._IsPingPointActive('__ANONYMOUS__');
    }
    ...
    Game_Interpreter.prototype.updateWaitMode = function () {
        if (this._waitMode === 'pingpoint') {
            if (FWD.IsPingPointActive(this._waitPingPointName)) {
                this._waitMode = '';
            ...
            if (FWD.IsAnonymousPingPointActive()) {
                this._waitMode = '';
            } else {
                return true;
            }
        } else if (this._waitMode === 'pingpointany') {
            if (FWD.IsAnyPingPointActive()) {
                this._waitMode = '';
            } else {
                return true;
            }
